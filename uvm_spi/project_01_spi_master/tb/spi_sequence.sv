...
class term_seq extends base_seq;
	...
	task body();
		...
		repeat (num) begin
			item = seq_item::type_id::create("item");
			start_item(item);
			if (!item.randomize() with {start dist {1:=1, 0:=9};}) `uvm_error(get_type_name(), "Randomize Failed")
			finish_item(item);
		end
	endtask
endclass
...
class direct_seq extends base_seq;
	...
	task body();
		...
		repeat (num) begin
			item = seq_item::type_id::create("item");
			start_item(item);
			if (!item.randomize() with {miso == 1'b1;}) `uvm_error(get_type_name(), "Randomize Failed")
			finish_item(item);
		end
	endtask
endclass
